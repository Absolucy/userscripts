...
setInterval(() => {
  const elements = document.querySelectorAll('svg.r-1janqcz.r-10ptun7, svg.r-14g73ha.r-1b94p3d');
  elements.forEach((node) => {
    if (node?.querySelector('path')?.getAttribute('d') === TOPIC_PATH) {
      ...
      if (p.tagName.toLowerCase() === 'article') {
        console.log(`NUKING TOPIC: ${p.textContent}`);
        p.remove();
      }
    }
  });

  const newStyleTopics = document.querySelectorAll("a[href^='/i/topics/'][id]");
  newStyleTopics.forEach((node) => {
    let p = node.parentElement;
    while (p.parentElement != null && p.tagName.toLowerCase() !== 'article') {
      p = p.parentElement;
    }
    if (p.tagName.toLowerCase() === 'article') {
      console.log(`NUKING NEW STYLE TOPIC: ${node.textContent}`);
      p.remove();
    }
  });
}, 500);
